...
export default async function run(servers: IServers, pathFiles: string) {
    ...
    try {
        ...
        for(let rc of arrServersGroupKey) {
            ...
            if(Array.isArray(servers[rc])) {    
                ...
                for(let index = 0; index < servers[rc].length; index++) {
                    ...
                    try {
                        ...
                        if(err instanceof ValidationError) {
                            ...
                            index--;
                            Log.instance().notify(err.name, err.title, err.message, err.stack!);
                        }
                    }
                }
                ...
                let arrDataCsv = await CSV.parse(`${pathFiles}\\${rc}\\${fileName}.csv`);
                ...
                if(!arrDataCsv.length) {
                    ...
                    Mail.instance.sendMail(title, message);
                ...
                servers[rc].forEach( (objServer, indexServer) => {
                ...
                console.log(serversIsNotVerified);console.log("||");console.log(serverVerified)
                if(serversIsNotVerified.size) {
                    ...
                    Mail.instance.sendMail("Servers not tested", `Servers that could not be checked:${servers.slice(0, -1)}`);
                }                     

                if(arrDataCsv.length) {
                    const unscheduledServersName = arrDataCsv.map( server => server.Server);
                    Mail.instance.sendMail("Unscheduled servers", `Servers that passed the test but are not in the .env file: ${unscheduledServersName}`)
                }
                                    
            } else {
                const titleErr = `Invalid data in .env file`;
                const objErr = new SyntaxErrorInFileENV(titleErr, `Invalid data in .env file: ${rc} is not Array!`);
